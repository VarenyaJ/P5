...
# Try to load docling; fall back gracefully if unavailable
DOC_AVAILABLE = False
try:
...
def _extract_with_docling(pdf_path: Path) -> tuple[str, str]:
    """
    Return (text, format_hint). Tries multiple docling APIs to be robust across versions.
    format_hint is "txt" or "md".
    """
    converter = DocumentConverter()
    res = converter.convert(str(pdf_path))

    # Some versions expose .document with export functions; others may expose helpers directly.
    obj = getattr(res, "document", res)

    # Prefer Markdown if available; otherwise text
    for attr, fmt in (("export_to_markdown", "md"), ("to_markdown", "md"),
                      ("export_to_text", "txt"), ("to_text", "txt")):
    ...
    # Last resort: stringify the structure
    return str(obj), "txt"
...
def main(input_path: str, out_dir: str, pattern: str, engine: str, dry_run: bool):
    ...
    click.echo(f"Found {len(files)} file(s). Using engine: {'docling' if use_docling else 'none'}")
    if dry_run:
        for p in files[:20]:
            click.echo(f"- {p}")
        if len(files) > 20:
        ...
        for p in files:
            ...
            try:
                ...
                if use_docling and p.suffix.lower() in VALID_SUFFIXES:
                    ...
                    # no engine, non-txt files: skip extraction but record manifest entry
                    pass

                if text:
                    out_name = p.with_suffix("." + fmt).name
                    target = text_dir / out_name
